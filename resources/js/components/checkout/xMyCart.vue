<template>
    <div class="row">
        ...
        <div class="col-sm-8 col-md-9" v-if="cart.cartCount > 0">
                ...
                <table class="table">
                    ...
                    <tr  v-for="product in product_cart" :key="product.key" v-if="product.item" >
                      ...
                      <small>{{ product.item.extraoption }}</small></span>
                        ...
                        <td><strong>{{ parseFloat(product.item.price).toFixed(3) }} {{$t('website.kd_label')}}</strong></td>
                        ...
                        <td><strong>{{ product.item.total.toFixed(3) }} {{$t('website.kd_label')}}</strong></td>
            ...
            <br>
            <a href="/checkout"><button class="btn-lg btn-primary rounded-0">{{$t('website.Proceed_To_Checkout')}}</button></a>
        </div><!--/.col-sm-9-->
        <div v-else class="col-sm-8 col-md-9">
            <a class="navbar-item" href="">
                {{$t('website.Cart is empty')}}Cart is empty
            </a>
        </div>
                ...
                <div class="data clearfix">
                    <div class="row">
                        ...
                        <div class="col-xs-6 list text-right">
                            {{ parseFloat(cart.cartbedforeTotal).toFixed(3) }} {{$t('website.kd_label')}}
                        </div>
                    </div>
                    ...
                    <div class="row" id="discountcodeDiv" v-if="discountamount>0"   >
                        ...
                        <div class="col-xs-6 list text-right">
                            <span id="discountcodeVal"> - {{ parseFloat(discountamount).toFixed(3) }}</span> {{$t('website.kd_label')}}
                        </div>
                    </div>
                            ...
                            <div class="total">
                                <div class="row">
                                    ...
                                    <div class="col-xs-6 text-right">
                                        {{ parseFloat(cart.cartTotal).toFixed(3) }} {{$t('website.kd_label')}}
                                    </div>
                                </div>
                            </div>
                        </div>
