...
$totalDiscount = 0 ;
        ...
        if ($discount ) {
            if ($discount->is_fixed) {
                ...
                $discount = $request * $discount->percentage / 100;
            }
        }
        else
        {
        ...
        <div class="col-sm-5">
            <div class="side-summary">
                ...
                <div class="data clearfix">
                    ...
                    <div class="row" id="discountcodeDiv" @if($discount < 1) style="display: none" @endif>
                        <div class="col-xs-6 list">
                ...
                <a class="heading" data-toggle="collapse" data-parent="#stacked-menu" href="#summary" aria-expanded="true">{{ $totalcount }} Items in Cart</a>
                <ul class="collapse in listing" id="summary" aria-expanded="true" style="">
                    ...
                    <li class="row">
                        ...
                        <div class="col-sm-4 pl-0 data">
                            ...
                            <span class="text-drkgreen">{{ $item->product->vendor->name }}</span>
                            <small> {{__('website.code_label')}}: {{ $item->product->code }}</small><br>
                            {{__('website.qty_label')}}: {{ $item->quantity }} <br>
                        </div>
                        <div class="col-sm-4 price">
                            <span class="green pull-right"> {{ number_format((float)($item->total), 2, '.', '') }} {{__('website.kd_label')}}</span>
                        </div>
                    </li><!--/li-->
                    @endforeach
                </ul><!--/nav-->
