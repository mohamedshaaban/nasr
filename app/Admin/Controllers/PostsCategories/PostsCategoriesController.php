...
namespace App\Admin\Controllers\PostsCategories;
...
class PostsCategoriesController extends Controller
{
    ...
    {
        $grid = new Grid(new PostsCategories);
    ...
    {
        $show = new Show(PostsCategories::findOrFail($id));
    ...
    {
        $form = new Form(new PostsCategories);

        $form->display('id', 'ID');
        $form->text('name_en', 'Title En')->required();
        $form->text('name_ar', 'Title Ar')->required();

//        $form->select('PostsCategories_id','PostsCategories')->options(PostsCategories::all()->pluck('name_en', 'id'));
